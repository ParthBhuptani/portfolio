    ...
    <header className="fixed inset-x-0 top-0 z-50 flex justify-center px-4 pt-4">
      <motion.nav
        ...
        className={`flex w-full max-w-3xl items-center justify-between rounded-full px-4 py-2 transition-all duration-300 ${
          scrolled
            ? "border bg-surface/90 backdrop-blur-md shadow-lg shadow-black/5"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="absolute top-20 w-[calc(100%-2rem)] max-w-3xl rounded-2xl border bg-surface/90 p-2 backdrop-blur-md md:hidden"
          >
            {links.map((l) => (
              <a
                key={l.href}
