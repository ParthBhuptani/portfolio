...
export function GitHubGraph() {
  ...
  useEffect(() => {
    ...
    fetch(`https://github-contributions-api.jogruber.de/v4/${site.github.username}?y=${year}`)
      .then((res) => {
              ...
              {weeks.map((week, w) => (
                <div key={w} className="flex flex-col gap-[3px]">
                  {week.map((day, d) => (
                    <span
                    key={day.date || `${w}-${d}`}
                      className={`h-[11px] w-[11px] flex-none rounded-[2px] transition-none ${SHADES[day.level]}`}
                      title={
